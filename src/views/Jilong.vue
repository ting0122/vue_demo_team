...
citys.value = data.filter(city => city.縣市名稱 === '基隆市'); // 先過濾出基隆市的資料
});
...
<div class="headline">基隆市瓦斯價格列表</div>
 ...
 <div class="list" v-else>
    <div class="product" v-for="city in citys" :key="city.縣市名稱">
      ...
      <div class="查報均價" v-if="city.縣市名稱 === '基隆市'">查報均價: {{ city.查報均價 }}元/20公斤</div>
      <div class="查報日期" v-if="city.縣市名稱 === '基隆市'">查報日期: {{ city.查報日期 }}</div>
    </div>
  </div>
