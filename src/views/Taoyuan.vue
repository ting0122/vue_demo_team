...
export default {
 ...
 created() {
    fetch("/gas_demand.json")  //fetch 操作處於 mounted 鉤子中
      .then(res => res.json())
      .then(data => {
        this.citys = data.filter(city => city.縣市名稱 === '桃園市');
      })
    fetch("/power.json")
      .then(res => res.json())
      .then(data => {
        this.pitys = data.filter(pity => pity.縣市名稱 === '桃園市');
      });
  }
};
...
border-radius: 8px; will-change: transform;">
 ...
 <div class="bigdev">
  ...
  <div class="list" v-else>
    <div class="txt">
      <div class="headline">桃園市瓦斯價格列表</div>
    </div>
   
    <div class="product" v-for="pity in pitys" :key="pity.縣市名稱">
      <div class="縣市名稱" v-if="pity.縣市名稱 === '桃園市'">{{ pity.縣市名稱 }}</div>
      <div class="查報均價" v-if="pity.縣市名稱 === '桃園市'">查報均價: {{ pity.查報均價 }}元/20公斤</div>
      <div class="查報日期" v-if="pity.縣市名稱 === '桃園市'">查報日期: {{ pity.查報日期 }}</div>
    </div>
  </div>
  <div class="headline1">桃園市瓦斯使用量列表</div>
  <div class="container2">
    <div class="list" v-if="this.citys === null">資料載入中</div>
    <div class="list" v-else>
      <div class="product" v-for="city in this.citys" :key="city.縣市名稱">
        <div class="縣市名稱" v-if="city.縣市名稱 === '桃園市'">{{ city.縣市名稱 }}</div>
        <div class="桶裝液化石油氣用量公噸" v-if="city.縣市名稱 === '桃園市'">桶裝液化石油氣用量公噸 {{ city.桶裝液化石油氣用量公噸 }}</div>
        <div class="年度" v-if="city.縣市名稱 === '桃園市'">年度: {{ city.年度 }}</div>
      </div>
    </div>
