...
export default {
  ...
  mounted() {
    fetch("./public/gas_demand.json")  //fetch 操作處於 mounted 鉤子中
      .then(res => res.json())
      .then(data => {
        this.citys = data.filter(city => city.縣市名稱 === '新竹縣');
      })
    fetch("./public/power.json")
      .then(res => res.json())
      .then(data => {
        this.pitys = data.filter(pity => pity.縣市名稱 === '新竹縣');
    });
  }
};
...
<div class="headline">新竹縣瓦斯價格列表</div>
 <div v-if="pitys===null">資料載入中</div> <!--設置空值，如果資料還未取得，就在畫面顯示 "資料載入中"  -->
 <div class="list" v-else>
    <div class="product" v-for="pity in pitys" :key="pity.縣市名稱">
      <div class="縣市名稱" v-if="pity.縣市名稱 === '新竹縣'">{{ pity.縣市名稱 }}</div>
      <div class="查報均價" v-if="pity.縣市名稱 === '新竹縣'">查報均價: {{ pity.查報均價 }}元/20公斤</div>
      <div class="查報日期" v-if="pity.縣市名稱 === '新竹縣'">查報日期: {{ pity.查報日期 }}</div>
    </div>
  </div>
 <div class="First">
    <div>
...
<div class="headline1">新竹縣瓦斯使用量列表</div>
<div class="container2">
    <div class="list" v-if="this.citys === null">資料載入中</div>
    <div class="list" v-else>
      <div class="product" v-for="city in this.citys" :key="city.縣市名稱">
        <div class="縣市名稱" v-if="city.縣市名稱 === '新竹縣'">{{ city.縣市名稱 }}</div>
        <div class="桶裝液化石油氣用量公噸" v-if="city.縣市名稱 === '新竹縣'">桶裝液化石油氣用量公噸 {{ city.桶裝液化石油氣用量公噸 }}</div>
        <div class="年度" v-if="city.縣市名稱 === '新竹縣'">年度: {{ city.年度 }}</div>
      </div>
